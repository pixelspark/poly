...
///
/// Based on LLM's original sampler (MIT/Apache2.0 licensed), see https://github.com/rustformers/llm/blob/main/crates/llm-base/src/samplers.rs
/// This version *adds* token biases to the predicted weights instead of replacing them.
// TODO: change this to simply use a token block/allowlist in a HashSet
#[derive(Clone, Debug)]
pub struct TopPTopKBiased {
...
impl Default for TopPTopKBiased {
	fn default() -> Self {
		Self {
			top_k: 40,
			top_p: 0.95,
...
impl Sampler for TopPTopKBiased {
	fn sample(&self, previous_tokens: &[TokenId], logits: &[f32], rng: &mut dyn rand::RngCore) -> TokenId {
		let Self {
			top_k,
			top_p,
			repeat_penalty,
		...
		// instead of having this monolithic function that embeds the repetition penalty and token bias
		{
			...
			for (i, &logit) in logits.iter().enumerate() {
				...
				let val = if let Some(logit_override) = bias_tokens.get(tid) {
					(logit + logit_override) * scale
				} else if previous_tokens[previous_tokens.len().saturating_sub(repetition_penalty_last_n)..].contains(&(i as TokenId)) {
		...
		let maxl = logits_id.iter().map(|x| x.0).max_by(f32::total_cmp).unwrap();

		// compute probs for the top K tokens
		let mut probs: Vec<f32> = logits_id.iter().copied().map(|(k, _)| (k - maxl).exp()).collect();
